...
const scaleWrapperEl = document.getElementById('scale-wrapper'); // New
const scrollWrapperEl = document.getElementById('scroll-wrapper'); // New
...
async function loadPDF(pdfUrl) {
    try {
        ...
        if (scaleWrapperEl) {
            scaleWrapperEl.style.width = '900px'; // Original width (450 * 2)
            scaleWrapperEl.style.height = '600px'; // Original height
        }
        ...
        for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
            ...
            // Using scale 2.0 ensures text stays crisp when zoomed in highly
...
function setupControls() {
    ...
    // --- NEW: Advanced Zoom Logic ---
    function updateZoom() {
        // 1. Scale the visual book
        bookContainerEl.style.transform = `scale(${currentZoom})`;
        
        // 2. Scale the physical wrapper to force scrollbars to appear
        if (scaleWrapperEl) {
    ...
    // --- NEW: Drag-to-Pan Functionality ---
    let isDown = false;
    let startX, startY, scrollLeft, scrollTop;

    scrollWrapperEl.addEventListener('mousedown', (e) => {
        // Only pan if they click the background, let the book handle its own page turns
